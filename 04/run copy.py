...
def readfile():
    ...
    for line in filter(lambda d:len(d)>1,lines):
        ...
        if len(boardlines)==5:
            ...
            boardlines=[]    
    [bingoboard,usedinputs,lastinput] = getBingoBoard(inputs, boards)
    print(str(bingoboard.sumUnmarked(usedinputs)*lastinput))

def getBingoBoard(inputs, boards):
    usedinputs=[]
    for input in inputs:
        usedinputs.append(input)
        for board in boards:
            boardisbingo = board.bingo(usedinputs)
            if len(boards)==1 & boards[0].bingo(usedinputs):
                return [boards[0],usedinputs,input]
            if boardisbingo:
                boards.remove(board)
